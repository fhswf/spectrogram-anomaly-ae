...
The Bosch dataset stores tri-axial acceleration samples in HDF5 files under `data/Mxx/OPxx/{good,bad}`. Use the preparation script to convert the HDF5 files into this repository's windowed `.npz` format:
...
| Notebook | Purpose |
|---|---|
| `notebooks/01_Load_Data_Segmentation_Labeling.ipynb` | Download/extract the original Mendeley dataset and create labeled fixed-sample-count vibration windows. |
...
The first notebook downloads the original Mendeley dataset, extracts the raw `.mat` files, and creates labeled fixed-sample-count vibration segments. The Bosch CNC dataset is prepared with `scripts/prepare_cnc_machining_dataset.py`, which uses the same target sample count by default and emits one full-file window when a CNC source file is shorter than the target.
